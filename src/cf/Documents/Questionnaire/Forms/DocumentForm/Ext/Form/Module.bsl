...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If (Not Object.Posted) And ValueIsFilled(Object.SectionToEdit) Then
		If TypeOf(Object.SectionToEdit) = Type("CatalogRef.QuestionnaireTemplateQuestions") Then
			CurrentSectionNumber = SurveysClientServer.FindStringInFormDataTree(SectionsTree, Object.SectionToEdit, "Ref", True);
		Else
			CurrentSectionNumber = SurveysClientServer.FindStringInFormDataTree(SectionsTree, Object.SectionToEdit, "RowType", True);
		EndIf;
...
Procedure RespondentStartChoice(Item, ChoiceData, StandardProcessing)
	...
	If RespondentsFilter.Count() > 0 Then
		FilterArray.Add(SurveysClient.CreateFilterParameterStructure(Type("DataCompositionFilterItem"),"Ref",DataCompositionComparisonType.InListByHierarchy,RespondentsFilter));
	EndIf;
...
// Event handler for basic questions and question charts.
&AtClient
...
// Handler that handlers the beginning of selecting text cells in question charts.
&AtClient
...
&AtClient
Procedure ResponseFormPostAndClose(Command)
...
Procedure SetAttributeValuesCompositeTabularQuestion(DoQueryBox,SelectionQuestion,TreeRow);
	...
	QuestionsArray1 = TreeRow.TableQuestionComposition.Unload().UnloadColumn("ElementaryQuestion");
	
	While SelectionQuestion.Next() Do
		
		If QuestionsArray1.Find(SelectionQuestion.ElementaryQuestion) = Undefined Then
			Continue;
		EndIf;
		
		If SelectionQuestion.CellNumber > Table.Count() Then
			
			For Indus = 1 To SelectionQuestion.CellNumber - Table.Count() Do
				Table.Add();
			EndDo;
			
		EndIf;
		
		QuestionNumberInArray = QuestionsArray1.Find(SelectionQuestion.ElementaryQuestion);
...
Procedure SetAttributeValuesTabularQuestionAnswersInRows(DoQueryBox,SelectionQuestion,TreeRow);
	...
	While SelectionQuestion.Next() Do
		...
		If (QuestionNumberInArray <> Undefined) And (SelectionQuestion.CellNumber <= Table.Count()) Then
			Table[SelectionQuestion.CellNumber - 1][NameOfColumnWithoutNumber + XMLString(QuestionNumberInArray+1)] = SelectionQuestion.Response;
		EndIf;
	...
	While SelectionQuestion.Next() Do
		...
		If (QuestionNumberInArray <> Undefined) Then
			If (QuestionNumberInArray <= Table.Count()) And (SelectionQuestion.CellNumber <= Table.Columns.Count()) Then
				Table[QuestionNumberInArray][NameOfColumnWithoutNumber + XMLString(SelectionQuestion.CellNumber + 1)] = SelectionQuestion.Response;
			EndIf;
		EndIf;
...
Procedure SetAttributeValuesTabularQuestionAnswersInRowsAndColumns(DoQueryBox,SelectionQuestion,TreeRow)
	...
	While SelectionQuestion.Next() Do
		If SelectionQuestion.ElementaryQuestion = QuestionCell Then
			ColumnNumber = ?(SelectionQuestion.CellNumber%(ColumnsCount - 1)=0,ColumnsCount - 1,SelectionQuestion.CellNumber%(ColumnsCount - 1));
...
Procedure SetAttributeValuesComplexQuestion(DoQueryBox,SelectionQuestion,TreeRow);
	...
	QuestionsArray1 = TreeRow.ComplexQuestionComposition.Unload().UnloadColumn("ElementaryQuestion");
	...
	While SelectionQuestion.Next() Do
		...
		If SelectionQuestion.ElementaryQuestion.ReplyType = Enums.TypesOfAnswersToQuestion.MultipleOptionsFor Then
			...
			OptionsOfAnswersToQuestion = Surveys.OptionsOfAnswersToQuestion(SelectionQuestion.ElementaryQuestion,ThisObject);
		...
		Else
			...
			If SelectionQuestion.ElementaryQuestion.ReplyType = Enums.TypesOfAnswersToQuestion.Text Then
...
// Converts questionnaire form data into table data.
//
&AtServer
	...
	For Each TableRow In SectionQuestionsTable Do
		...
		If ValueIsFilled(TableRow.ParentQuestion) Then
			...
			If FoundRows.Count() > 0 Then
				...
				If (Not ThisObject[SurveysClientServer.GetQuestionName(ParentRow.Composite)] = True) Then
					Continue;
				EndIf;
			EndIf;
		EndIf;
...
Procedure FillAnswersTabularQuestionAnswersInRows(TreeRow,Table)
	...
	NameOfColumnWithoutNumber = SurveysClientServer.GetQuestionName(TreeRow.Composite) + "TableColumn";
...
Procedure FillAnswersTabularQuestionAnswersInRowsAndColumns(TreeRow,Table)
	...
	RowsAnswers  = TreeRow.PredefinedAnswers.FindRows(New Structure("ElementaryQuestion",QuestionForRows));
	ColumnsAnswers = TreeRow.PredefinedAnswers.FindRows(New Structure("ElementaryQuestion",QuestionForColumns));
...
Procedure FillAnswersComplexQuestion(TreeRow)
	...
	For Each ComplexQuestionRow In TreeRow.ComplexQuestionComposition Do
		...
		If ComplexQuestionRow.ElementaryQuestion.ReplyType <> Enums.TypesOfAnswersToQuestion.MultipleOptionsFor Then
		...
		Else
			...
			OptionsOfAnswersToQuestion = Surveys.OptionsOfAnswersToQuestion(ComplexQuestionRow.ElementaryQuestion,ThisObject);
...
Function CheckQuestionnaireFilling()
	...
	While Selection.Next() Do
		Common.MessageToUser(NStr("en = 'Reply to the question is not specified.';") + "- "+ StrReplace(Selection.Ref.FullCode(),"/",".") + " " + Selection.Wording);
	EndDo;
...
Function FindAnswerInArray(Response,AnswersArray)
	...
	For Indus = 1 To AnswersArray.Count() Do
		
		If AnswersArray[Indus - 1].Response = Response Then
			
			ReturnStructure.Insert("AttributeSequenceNumber",Indus);
			ReturnStructure.Insert("OpenEndedQuestion",AnswersArray[Indus - 1].OpenEndedQuestion);
	...
	For Each CollectionItem In DependentQuestions Do
		...
		For Each SubordinateQuestion In CollectionItem.SubordinateItems Do
			...
			If StrOccurrenceCount(SubordinateQuestion.SubordinateQuestionItemName, "Attribute") = 0 Then
				Try
					ItemOfSubordinateQuestion.AutoMarkIncomplete = ThisObject[QuestionName] And SubordinateQuestion.IsRequired;
				Except
					// 
				EndTry;
			EndIf;
...
// Sets an event handler for basic questions and question charts.
&AtServer
	...
	For Each TableRow In SectionQuestionsTable Do
		...
		Else
			If TableRow.ReplyType = Enums.TypesOfAnswersToQuestion.MultipleOptionsFor Then
				OptionsOfAnswersToQuestion = Surveys.OptionsOfAnswersToQuestion(TableRow.ElementaryQuestion,ThisObject);
				For Indus = 1  To OptionsOfAnswersToQuestion.Count() Do
					Items[QuestionName + "_Attribute_" + Indus].SetAction("OnChange","Attachable_OnChangeQuestion");
					If OptionsOfAnswersToQuestion[Indus-1].OpenEndedQuestion Then
						Items[QuestionName + "_Comment_" + Indus].SetAction("OnChange","Attachable_OnChangeQuestion");
					EndIf;
				EndDo;
			Else
...
Procedure SetOnChangeEventHandlerForComplexQuestions(TableRow, QuestionName)
	...
	For Each ComplexQuestionRow In TableRow.ComplexQuestionComposition Do
		...
		FoundRows = QuestionsPresentationTypes.FindRows(New Structure("DoQueryBox",ComplexQuestionRow.ElementaryQuestion));
		If FoundRows.Count() > 0 Then
		...
		If ElementaryQuestionAttributes.ReplyType = Enums.TypesOfAnswersToQuestion.MultipleOptionsFor Then
			OptionsOfAnswersToQuestion = Surveys.OptionsOfAnswersToQuestion(ComplexQuestionRow.ElementaryQuestion,ThisObject);
			For Indus = 1  To OptionsOfAnswersToQuestion.Count() Do
				QuestionAttributeName1 = QuestionName + "_Response_" + Format(ComplexQuestionRow.LineNumber, "NG=") + "_Attribute_" + Indus;
				Items[QuestionAttributeName1].SetAction("OnChange","Attachable_OnChangeQuestion");
				If OptionsOfAnswersToQuestion[Indus-1].OpenEndedQuestion Then
					CommentAttributeName = QuestionName + "_Response_" + Format(ComplexQuestionRow.LineNumber, "NG=") + "_Comment_" + Indus;
...
Procedure ChangeSectionsTreeVisibility()
	...
	TitleSectionsCommand = ?(Items.SectionsTreeGroup.Visible,NStr("en = 'Hide sections';"),NStr("en = 'Show sections';"));
	Items.HideShowSectionsTreeDocument.Title = TitleSectionsCommand;
