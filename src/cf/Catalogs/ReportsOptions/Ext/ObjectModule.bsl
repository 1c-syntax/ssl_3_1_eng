...
Procedure OnWrite(Cancel)
	...
	OptionUsers = CommonClientServer.StructureProperty(AdditionalProperties, "OptionUsers");
	IsNew = CommonClientServer.StructureProperty(AdditionalProperties, "IsNew", False);
	NotifyUsers = CommonClientServer.StructureProperty(AdditionalProperties, "NotifyUsers", False);
...
Procedure FillInThePlacementByParent()
	...
	If Location.Count() > 0
		Or Not ValueIsFilled(Parent) Then 
	...
	If Location.Count() > 0
		Or Not ValueIsFilled(ParentProperties.PredefinedOption) Then 
