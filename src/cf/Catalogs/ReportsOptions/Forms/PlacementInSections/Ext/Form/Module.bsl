	...
	If ErrorsMessages <> Undefined Then
		...
		StandardSubsystemsClient.ShowQuestionToUser(Undefined, 
			StringFunctionsClientServer.SubstituteParametersToString(NStr("en = '%1
				|Details:
				|%2';"), ErrorsMessages.Text, ErrorsMessages.More), QuestionDialogMode.OK);
	EndIf;
EndProcedure
...
Procedure Place(Command)
	...
	NotificationText1 = StringFunctionsClientServer.SubstituteParametersToString(NotificationText1, Format(OptionsToAssign.Count(), "NZ=0; NG=0"));
...
Procedure FillSections()
	...
	If ErrorsCount > 0 Then
		...
		For Each TableRow In FilteredOptions Do
			...
			ErrorsMessages.More = TrimL(ErrorsMessages.More) + Chars.LF + "    - " + String(TableRow.Ref);
