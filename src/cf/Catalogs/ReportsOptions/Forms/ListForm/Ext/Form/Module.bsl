...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If CurrentLanguageSuffix <> Undefined Then
		...
		If ValueIsFilled(CurrentLanguageSuffix) Then
			...
			ListProperties.QueryText = StrReplace(List.QueryText, "ReportsOptions.Description", "ReportsOptions.Description" + CurrentLanguageSuffix);
			ListProperties.QueryText = StrReplace(ListProperties.QueryText, "ReportsOptions.LongDesc", "ReportsOptions.LongDesc" + CurrentLanguageSuffix);
			
			ListProperties.QueryText = StrReplace(ListProperties.QueryText, "ConfigurationOptions.Description", "ConfigurationOptions.Description" + CurrentLanguageSuffix);
			ListProperties.QueryText = StrReplace(ListProperties.QueryText, "ConfigurationOptions.LongDesc", "ConfigurationOptions.LongDesc" + CurrentLanguageSuffix);
	...
	If Common.SubsystemExists("StandardSubsystems.AdditionalReportsAndDataProcessors") Then
		...
		DescriptionFieldsReportName = 
			StringFunctionsClientServer.SubstituteParametersToString("CASE
			|	WHEN VALUETYPE(ReportsOptionsOverridable.Report) = TYPE(Catalog.MetadataObjectIDs)
			|		THEN CAST(ReportsOptionsOverridable.Report AS Catalog.MetadataObjectIDs).Name
			|	WHEN VALUETYPE(ReportsOptionsOverridable.Report) = TYPE(Catalog.ExtensionObjectIDs)
			|		THEN CAST(ReportsOptionsOverridable.Report AS Catalog.ExtensionObjectIDs).Name
...
Procedure SaveReportOptionToFile(Command)
	...
	OpenForm("SettingsStorage.ReportsVariantsStorage.Form.SaveReportOptionToFile",
		New Structure("SelectedReportsOptions", SelectedReportsOptions),
		ThisObject);
EndProcedure
...
Procedure SubsystemsTreeDragCompletion(Response, PlacementParameters) Export
	...
	If PlacementParameters.Variants.Total = ExecutionResult.Placed Then
		...
		Else
			...
			EndIf;
			Text = StringFunctionsClientServer.SubstituteParametersToString(NStr("en = '%1 report options.';"), Format(PlacementParameters.Variants.Total, "NZ=0; NG=0"));
			Ref = Undefined;
		EndIf;
		Template = StringFunctionsClientServer.SubstituteParametersToString(Template, PlacementParameters.Receiver.FullPresentation);
	...
	Else
		...
		If Not IsBlankString(ExecutionResult.CannotBePlaced) Then
			ErrorsText = ?(ErrorsText = "", "", ErrorsText + Chars.LF + Chars.LF)
				+ NStr("en = 'Cannot assign to the command interface:';")
				+ Chars.LF
				+ ExecutionResult.CannotBePlaced;
		EndIf;
		If Not IsBlankString(ExecutionResult.AlreadyAssigned) Then
			ErrorsText = ?(ErrorsText = "", "", ErrorsText + Chars.LF + Chars.LF)
				+ NStr("en = 'Already assigned to this section:';")
				+ Chars.LF
				+ ExecutionResult.AlreadyAssigned;
		EndIf;
