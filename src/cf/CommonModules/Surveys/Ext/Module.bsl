...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment = NStr("en = 'New attribute ""Kind"" is being populated in the ""Questionnaire"" documents.
		|Until the population is complete, the attribute might be displayed incorrectly.';");
	Handler.ExecutionMode = "Deferred";
	Handler.DeferredProcessingQueue = 1;
...
Procedure UpdateTabularQuestionPreview(Questions,Replies,TabularQuestionType,Form,TableNamePreview,Var_Key) Export
	...
	If TabularQuestionType = Enums.TabularQuestionTypes.Composite Then 
		...
		While SelectionQuestions.Next() Do
			...
			ResultingTableServer.Columns.Add(NameOfColumnWithoutNumber + XMLString(ColumnsCounter),SelectionQuestions.ValueType,SelectionQuestions.Wording);
			AttributesToBeAdded.Add(New FormAttribute(NameOfColumnWithoutNumber + XMLString(ColumnsCounter),SelectionQuestions.ValueType,TableNamePreview));
	...
	ElsIf TabularQuestionType = Enums.TabularQuestionTypes.PredefinedAnswersInColumns Then
		...
		If SelectionQuestions.Next() Then
			...
			AttributesToBeAdded.Add(New FormAttribute(NameOfColumnWithoutNumber + "1",TypesDetailsString,TableNamePreview));
			
		EndIf;
		
		For Indus = 1 To Replies.Count() Do
			
			ResultingTableServer.Columns.Add(NameOfColumnWithoutNumber + XMLString(Indus + 1),TypesDetailsString,Replies[Indus-1].Response);
			AttributesToBeAdded.Add(New FormAttribute(NameOfColumnWithoutNumber + XMLString(Indus + 1),TypesDetailsString,TableNamePreview));
	...
	ElsIf TabularQuestionType = Enums.TabularQuestionTypes.PredefinedAnswersInRowsAndColumns Then
		...
		AttributesToBeAdded.Add(New FormAttribute(NameOfColumnWithoutNumber + "1",QuestionsTable[0].ValueType,TableNamePreview));
		...
		For Indus = 1 To AnswersInColumns.Count() Do
			...
			ResultingTableServer.Columns.Add(NameOfColumnWithoutNumber + XMLString(Indus + 1),QuestionsTable[2].ValueType,AnswersInColumns[Indus-1].Response);
			AttributesToBeAdded.Add(New FormAttribute(NameOfColumnWithoutNumber + XMLString(Indus + 1),QuestionsTable[2].ValueType,TableNamePreview));
	...
	For Indus = 1 To AttributesToBeAdded.Count() Do
		...
		Item = Form.Items.Add(NameOfColumnWithoutNumber + XMLString(Indus), Type("FormField"), Form.Items[TableNamePreview]);
		...
		If (TabularQuestionType = Enums.TabularQuestionTypes.Composite 
			Or TabularQuestionType = Enums.TabularQuestionTypes.PredefinedAnswersInRows)
			Or (Indus > 1) Then
...
Procedure AddQuestionnaireTreeRows(Selection,ParentRow, RecursionLevel, Form)
	...
	While Selection.Next() Do
		...
		NewRow.PictureCode                = SurveysClientServer.GetQuestionnaireTemplatePictureCode(Selection.IsSection,Selection.QuestionType);
		...
		If Form.FormName = "Catalog.QuestionnaireTemplates.Form.ItemForm" Then
			NewRow.ElementaryQuestion = ?(Selection.IsSection,Selection.Description,?(Selection.QuestionType <> Enums.QuestionnaireTemplateQuestionTypes.Tabular,Selection.ElementaryQuestion,Selection.Wording));
			NewRow.IsRequired       = ?(Selection.IsSection,Undefined,?(Selection.QuestionType <> Enums.QuestionnaireTemplateQuestionTypes.Tabular,Selection.IsRequired,Undefined)); 
...
Procedure CreateFillingFormBySection(Form, CurrentDataSectionsTree) Export
	...
	Else
		...
		Introduction = ?(IsBlankString(Form.Introduction), NStr("en = 'Click Next to fill out the questionnaire.';"), Form.Introduction);
		ClosingStatement = ?(IsBlankString(Form.ClosingStatement), NStr("en = 'Thank you for filling out the questionnaire.';"), Form.ClosingStatement);
		
		ItemLabelIntroduction.Title = ?(CurrentDataSectionsTree.RowType = "Introduction", Introduction, ClosingStatement);
		ItemLabelIntroduction.TextColor = StyleColors.FieldTextColor;
		ItemLabelIntroduction.Font = StyleFonts.SmallTextFont;
...
Procedure AddAttributesTabularQuestion(TreeRow, AttributesToBeAdded, Form)
	...
	If TabularQuestionType = Enums.TabularQuestionTypes.Composite
		...
		For Indus = 1 To TreeRow.TableQuestionComposition.Count() Do
			...
			FoundRows = Form.QuestionsPresentationTypes.FindRows(New Structure("DoQueryBox",TreeRow.TableQuestionComposition[Indus-1].ElementaryQuestion));
	...
	ElsIf TabularQuestionType = Enums.TabularQuestionTypes.PredefinedAnswersInColumns Then
		...
		// 
		AttributesToBeAdded.Add(New FormAttribute(NameOfColumnWithoutNumber + "1",New TypeDescription("ChartOfCharacteristicTypesRef.QuestionsForSurvey"),TableName));
		
		// Add other columns.
		AnswersArray1 = TreeRow.PredefinedAnswers.FindRows(New Structure("ElementaryQuestion",QuestionForColumns));
	...
	ElsIf TabularQuestionType = Enums.TabularQuestionTypes.PredefinedAnswersInRowsAndColumns Then
		...
		EndIf;
		// 
		AttributesToBeAdded.Add(New FormAttribute(NameOfColumnWithoutNumber + "1",QuestionTypePresentationForRows.Type,TableName,QuestionTypePresentationForRows.Wording));
...
Procedure AddAttributesComplexQuestion(TreeRow, AttributesToBeAdded, Form)
	...
	For Each ComplexQuestionRow In TreeRow.ComplexQuestionComposition Do // LineOfATabularSection: см. СправочникТабличнаяЧасть.ВопросыШаблонаАнкеты.СоставКомплексногоВопроса
		...
		FoundRows = Form.QuestionsPresentationTypes.FindRows(New Structure("DoQueryBox",ComplexQuestionRow.ElementaryQuestion));
		If FoundRows.Count() > 0 Then
			QuestionTypePresentation = FoundRows[0];
		Else
			Continue;
		...
		If QuestionTypePresentation.ReplyType = Enums.TypesOfAnswersToQuestion.MultipleOptionsFor Then
			...
			For Each AnswerOption In OptionsOfAnswersToQuestion Do
				...
				Counter = Counter + 1;
				AttributesToBeAdded.Add(New FormAttribute(QuestionName + "_Response_" 
					+ Format(ComplexQuestionRow.LineNumber, "NG=") + "_Attribute_"
					+ Counter,BooleanTypeDetails,,AnswerOption.Presentation));
		...
		If (QuestionTypePresentation.ReplyType <> Enums.TypesOfAnswersToQuestion.MultipleOptionsFor) And (ComplexQuestionRow.CommentRequired) Then
			
			RowTypeDetails = New TypeDescription("String",,New StringQualifiers(150));
			AttributesToBeAdded.Add(New FormAttribute(QuestionName + "_Comment_" 
...
Procedure PositionOnFirstSectionQuestion(Form)
	...
	If Form.SectionQuestionsTable.Count() > 0 Then
		...
		QuestionName = SurveysClientServer.GetQuestionName(Form.SectionQuestionsTable[0].Composite);
...
Procedure AddQuestionItems(TableRow, GroupItem1, Form)
	...
	QuestionName = SurveysClientServer.GetQuestionName(TableRow.Composite);
	...
	QuestionGroupItemComment = Form.Items.Add(QuestionName + "GroupQuestionComment",Type("FormGroup"),QuestionGroupItem);
	...
	Else
		...
		ElsIf TableRow.ReplyType = Enums.TypesOfAnswersToQuestion.Number Then
			...
			If TableRow.MinValue <> 0 Or TableRow.MaxValue <> 0 Then
				...
				If TableRow.MinValue <> 0 And TableRow.MaxValue <> 0 Then
					ToolTipText = StringFunctionsClientServer.SubstituteParametersToString(NStr("en = 'You can enter value from %1 to %2';"),
						TableRow.MinValue, TableRow.MaxValue);
				ElsIf TableRow.MinValue = 0 And TableRow.MaxValue <> 0 Then
					ToolTipText = StringFunctionsClientServer.SubstituteParametersToString(NStr("en = 'You can enter value to %1';"), TableRow.MaxValue);
				Else
					ToolTipText = StringFunctionsClientServer.SubstituteParametersToString(NStr("en = 'You can enter value from %1';"), TableRow.MinValue);
				EndIf;
		...
		ElsIf TableRow.ReplyType = Enums.TypesOfAnswersToQuestion.MultipleOptionsFor Then
			...
			AnswersOptionsGroupItem = Form.Items.Add(QuestionName + "GroupOptions",Type("FormGroup"),QuestionGroupItemComment);
			...
			For Each AnswerOption In OptionsOfAnswersToQuestion Do
				...
				AnswerOptionGroupItem = Form.Items.Add(QuestionName + "GroupResponseOption" + XMLString(Counter),
					Type("FormGroup"), AnswersOptionsGroupItem);
		...
		If (TableRow.ReplyType <> Enums.TypesOfAnswersToQuestion.MultipleOptionsFor) And (TableRow.CommentRequired) Then
...
Procedure AddTabularQuestionItems(TableRow, GroupItem1, Form)
	...
	If TableRow.TabularQuestionType = Enums.TabularQuestionTypes.PredefinedAnswersInRowsAndColumns 
		...
		FoundRows = Form.QuestionsPresentationTypes.FindRows(New Structure("DoQueryBox",TableRow.TableQuestionComposition[2].ElementaryQuestion));
		If FoundRows.Count() > 0 Then
	...
	For Indus = 1 To QuestionTable.Columns.Count() Do
		...
		If TableRow.TabularQuestionType <> Enums.TabularQuestionTypes.PredefinedAnswersInRowsAndColumns 
			...
			FoundRows = Form.QuestionsPresentationTypes.FindRows(New Structure("DoQueryBox",TableRow.TableQuestionComposition[Indus-1].ElementaryQuestion));
			If FoundRows.Count() > 0 Then
				ElementaryQuestionAttributes = FoundRows[0];
			Else
				Continue;
		...
		Else
			...
			If TableRow.TabularQuestionType = Enums.TabularQuestionTypes.Composite 
				...
				If QuestionTable.Columns[ColumnName].ValueType = New TypeDescription("CatalogRef.QuestionnaireAnswersOptions") Then
					...
					OptionsOfAnswersToQuestion = OptionsOfAnswersToQuestion(TableRow.TableQuestionComposition[Indus-1].ElementaryQuestion,Form);
			...
			ElsIf TableRow.TabularQuestionType = Enums.TabularQuestionTypes.PredefinedAnswersInRowsAndColumns Then	
				...
				If TableRow.TableQuestionComposition.Count() = 3 And Indus <> 1 Then
					...
					If TableRow.TableQuestionComposition[2].ElementaryQuestion.ValueType = New TypeDescription("CatalogRef.QuestionnaireAnswersOptions")Then
						
						Item.ListChoiceMode = True;
						OptionsOfAnswersToQuestion = OptionsOfAnswersToQuestion(TableRow.TableQuestionComposition[2].ElementaryQuestion,Form);
						For Each OptionOfAnswerToQuestion In OptionsOfAnswersToQuestion Do
							ItemSelectionList = Item.ChoiceList; // ValueList
							ItemSelectionList.Add(OptionOfAnswerToQuestion.Response);
						EndDo;
		...
		If (TabularQuestionType = Enums.TabularQuestionTypes.PredefinedAnswersInColumns)Then
			...
			Else	
				...
				ChoiceParameterLinks.Add(New ChoiceParameterLink("Filter.Owner","Items." + TableName + ".CurrentData." + TableName + "_Column_1"));
	...
	If TabularQuestionType = Enums.TabularQuestionTypes.PredefinedAnswersInRows Then
		...
		AnswersArray1 = TableRow.PredefinedAnswers.FindRows(New Structure("ElementaryQuestion",TableRow.TableQuestionComposition[0].ElementaryQuestion));
...
Procedure AddComplexQuestionItems(TableRow, GroupItem1, Form)
	...
	For Each ComplexQuestionRow In TableRow.ComplexQuestionComposition Do // LineOfATabularSection: см. СправочникТабличнаяЧасть.ВопросыШаблонаАнкеты.СоставКомплексногоВопроса
		...
		If ElementaryQuestionAttributes.ReplyType = Enums.TypesOfAnswersToQuestion.Boolean Then
			Item = Form.Items.Add(
				QuestionName + "_ElementaryQuestion_" + Format(ComplexQuestionRow.LineNumber, "NG="), 
				Type("FormDecoration"),QuestionGroupItemBoolean);
		Else
			Item = Form.Items.Add(
				QuestionName + "_ElementaryQuestion_" + Format(ComplexQuestionRow.LineNumber, "NG="),
				Type("FormDecoration"),GroupItem1);
		EndIf;
		Item.Type                        = FormDecorationType.Label;
		Item.Title                  = ComplexQuestionRow.ElementaryQuestion;
		Item.AutoMaxWidth     = False;
		Item.HorizontalStretch   = (ElementaryQuestionAttributes.ReplyType <> Enums.TypesOfAnswersToQuestion.Boolean);
		Item.Font = StyleFonts.SmallTextFont;
		
		If ElementaryQuestionAttributes.ReplyType = Enums.TypesOfAnswersToQuestion.String Then
			
			Item = Form.Items.Add(QuestionName + "_Response_" + Format(ComplexQuestionRow.LineNumber, "NG="),Type("FormField"),GroupItem1);
			Item.Type                        = FormFieldType.InputField;
			Item.TitleLocation         = FormItemTitleLocation.None;
			Item.AutoMarkIncomplete  = TableRow.IsRequired;
		...
		ElsIf ElementaryQuestionAttributes.ReplyType = Enums.TypesOfAnswersToQuestion.Number Then
			...
			If ElementaryQuestionAttributes.MinValue <> 0 Or ElementaryQuestionAttributes.MaxValue <> 0 Then
				...
				ElsIf ElementaryQuestionAttributes.MinValue = 0 And ElementaryQuestionAttributes.MaxValue <> 0 Then
					ToolTipText = StringFunctionsClientServer.SubstituteParametersToString(NStr("en = 'You can enter value to %1';"), ElementaryQuestionAttributes.MaxValue);
				Else
					ToolTipText = StringFunctionsClientServer.SubstituteParametersToString(NStr("en = 'You can enter value from %1';"), ElementaryQuestionAttributes.MinValue);
				EndIf;
		...
		ElsIf ElementaryQuestionAttributes.ReplyType = Enums.TypesOfAnswersToQuestion.MultipleOptionsFor Then
			...
			AnswersOptionsGroupItem = Form.Items.Add(QuestionName + "_Response_" 
				+ Format(ComplexQuestionRow.LineNumber, "NG=") + "GroupOptions",Type("FormGroup"),GroupItem1);
			...
			For Each AnswerOption In OptionsOfAnswersToQuestion Do
				...
				AnswerOptionGroupItem = Form.Items.Add(QuestionName + "_Response_" 
					+ Format(ComplexQuestionRow.LineNumber, "NG=") + "GroupResponseOption" + XMLString(Counter),
					Type("FormGroup"),AnswersOptionsGroupItem);
				...
				QuestionAttributeName1 = QuestionName + "_Response_" + Format(ComplexQuestionRow.LineNumber, "NG=") + "_Attribute_" + Counter;
				Item = Form.Items.Add(QuestionAttributeName1,Type("FormField"),AnswerOptionGroupItem);
		...
		// Comments to questions.
		If (ElementaryQuestionAttributes.ReplyType <> Enums.TypesOfAnswersToQuestion.MultipleOptionsFor) And (ComplexQuestionRow.CommentRequired) Then
			
			Item = Form.Items.Add(QuestionName + "_Comment_" 
				+ Format(ComplexQuestionRow.LineNumber, "NG="),Type("FormField"),GroupItem1);
...
Procedure GetInformationOnQuestionnaireQuestions(Form, QuestionnaireTemplate, Section,FullSectionCode)
	...
	While QuestionsByQuestionnaireSectionSelection.Next() Do
		...
		FillPropertyValues(NewRow,QuestionsByQuestionnaireSectionSelection,,"TableQuestionComposition,PredefinedAnswers,ComplexQuestionComposition");
...
Procedure GenerateQuestionsSubordinationTable(Form) Export
	...
	While Selection.Next() Do
		...
		Form.Items[SurveysClientServer.GetQuestionName(Selection.ParentSrtingKey)].SetAction("OnChange","Attachable_OnChangeQuestionsWithConditions");
		...
		While DetailsSelection.Next() Do
			...
			QuestionName = SurveysClientServer.GetQuestionName(DetailsSelection.Composite);
...
// Gets presentations of elementary questions in a question chart
// and populates the QuestionsPresentation map
