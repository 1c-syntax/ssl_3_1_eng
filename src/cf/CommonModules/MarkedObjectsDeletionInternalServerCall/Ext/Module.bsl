...
Procedure SetDeleteOnScheduleMode(Changes) Export
	...
	JobID = ScheduledJobsServer.UUID(Metadata.ScheduledJobs.MarkedObjectsDeletion);
...
Function ModeDeleteOnSchedule() Export
	...
	Result.Insert("Schedule", CommonClientServer.ScheduleToStructure(New JobSchedule()));
	...
	If Jobs.Count() > 0 Then
		...
		Else	
			Result.Schedule = ScheduledJobsServer.JobSchedule(Jobs.UUID, True);
		EndIf;
	EndIf;
