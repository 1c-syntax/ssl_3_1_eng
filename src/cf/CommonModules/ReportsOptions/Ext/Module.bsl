...
//       * OptionRef1 - CatalogRef.ReportsOptions - Report option reference.
...
Function AttachReportAndImportSettings(Parameters) Export
		...
		If TypeOf(Result.OptionRef1) <> Type("CatalogRef.ReportsOptions")
			...
			If Result.OptionRef1 = Undefined Then
				Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("en = 'Parameters %2 are not provided in method %1.';"),
					"AttachReportAndImportSettings",
					"OptionRef1, RefOfReport, VariantKey");
				Return Result;
			EndIf;
		EndIf;
		PropertiesNames = "VariantKey, Parent, Parent.VariantKey" + ?(MustReadReportRef, ", Report", "") + ?(MustReadSettings, ", Settings", "");
	...
	If Not SchemaURLFilled And TypeOf(ReportObject.DataCompositionSchema) = Type("DataCompositionSchema") Then
		FormIdentifier = CommonClientServer.StructureProperty(Parameters, "FormIdentifier");
	...
	For Each ReportVariant In AdditionalReportOptions Do
		...
		If Presentation <> Undefined Then
			...
			If Common.SubsystemExists("StandardSubsystems.NationalLanguageSupport") Then
				...
				ModuleNativeLanguagesSupportServer.OnReadPresentationsAtServer(OptionObject);
			EndIf;
...
Function GenerateReport(Val Parameters, Val CheckFilling, Val GetCheckBoxEmpty) Export
	...
	AuxProperties.Insert("TablesToUse",
		CommonClientServer.StructureProperty(Parameters, "TablesToUse", New Array));
	...
	If Not Common.DataSeparationEnabled()
		...
		HeaderFooterManagement.SetHeadersAndFooters(
			Result.SpreadsheetDocument, ReportTitle,, HeaderOrFooterSettings);
	EndIf;
...
Function PredefinedReportsOptions(ReportsType = "BuiltIn", ConnectedToTheStorage = True) Export
	...
	For Each ReportMetadata In Metadata.Reports Do 
		...
		If IsBlankString(DescriptionOfReport.MainOption) Then
			OptionDetails = Result.Add();
			FillPropertyValues(OptionDetails, DescriptionOfReport, "Report, Description");
...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment = NStr("en = 'Configuring data source in universal report option settings.
...
Procedure GenerateReportInBackground(Parameters, StorageAddress) Export
	Generation1 = GenerateReport(Parameters, False, False);
	
	Result = New Structure("SpreadsheetDocument, Details,
		|Success, ErrorText, DataStillUpdating,
		|VariantModified, UserSettingsModified");
	FillPropertyValues(Result, Generation1);
	
	If Result.VariantModified Or Parameters.ParametersChanged Then
		Result.Insert("DCSettings", Generation1.DCSettings);
	EndIf;
	If Result.UserSettingsModified Then
		Result.Insert("DCUserSettings", Generation1.DCUserSettings);
	EndIf;
...
Function CommonPanelSettings() Export
	...
	If CommonSettings = Undefined Then
		CommonSettings = New Structure("ShowTooltips, SearchInAllSections");
...
Function UpdatePredefinedReportOption(Mode, OptionDetails, Result)
	...
	Try
		...
		If Common.SubsystemExists("StandardSubsystems.NationalLanguageSupport") Then
			...
			If ModuleNativeLanguagesSupportServer.SecondAdditionalInfobaseLanguageCode() = CurrentLanguage().LanguageCode Then
...
Procedure UpdateReportsOptionsByPredefinedOnes(Mode, Result)
	...
	AttributesToChange = New Structure("DeletionMark, Parent,
		|Description, Report, VariantKey, PredefinedOption");
...
Function FillFieldsForSearch(OptionObject, ReportInfo = Undefined) Export
	...
	If TypeOf(DCSettings) <> Type("DataCompositionSettings")
		...
		Except
			MessageTemplate = NStr("en = 'Cannot read settings of a custom report option:
				|%1.';");
...
Function FindReportsOptions(Val SearchParameters, Val GetSummaryTable = False, Val GetHighlight = False) Export
	...
	If Common.SubsystemExists("StandardSubsystems.NationalLanguageSupport") Then
		...
		If ValueIsFilled(CurrentLanguageSuffix) Then
			...
			ModuleNativeLanguagesSupportServer.ChangeRequestFieldUnderCurrentLanguage(QueryText, "ReportsOptions.Description");
			ModuleNativeLanguagesSupportServer.ChangeRequestFieldUnderCurrentLanguage(QueryText, "ReportsOptions.LongDesc");
			ModuleNativeLanguagesSupportServer.ChangeRequestFieldUnderCurrentLanguage(QueryText, "ConfigurationOptions.Description");
			ModuleNativeLanguagesSupportServer.ChangeRequestFieldUnderCurrentLanguage(QueryText, "ConfigurationOptions.LongDesc");
			
			ModuleNativeLanguagesSupportServer.ChangeRequestFieldUnderCurrentLanguage(QueryText, "ExtensionOptions.Description");
			ModuleNativeLanguagesSupportServer.ChangeRequestFieldUnderCurrentLanguage(QueryText, "ExtensionOptions.LongDesc");
			ModuleNativeLanguagesSupportServer.ChangeRequestFieldUnderCurrentLanguage(QueryText, "ExtensionOptions.FieldDescriptions");
			ModuleNativeLanguagesSupportServer.ChangeRequestFieldUnderCurrentLanguage(QueryText, "ExtensionOptions.FilterParameterDescriptions");
			ModuleNativeLanguagesSupportServer.ChangeRequestFieldUnderCurrentLanguage(QueryText, "ExtensionOptions.Keywords");
		...
		TheTextOfTheRequest = "
			...
			If ValueIsFilled(CurrentLanguageSuffix) Then
				If Common.SubsystemExists("StandardSubsystems.NationalLanguageSupport") Then
					...
					ModuleNativeLanguagesSupportServer.ChangeRequestFieldUnderCurrentLanguage(TheTextOfTheRequest, "ReportsOptions.Description");
					ModuleNativeLanguagesSupportServer.ChangeRequestFieldUnderCurrentLanguage(TheTextOfTheRequest, "ReportsOptions.LongDesc");
					
					ModuleNativeLanguagesSupportServer.ChangeRequestFieldUnderCurrentLanguage(TheTextOfTheRequest, "ConfigurationOptions.Description");
					ModuleNativeLanguagesSupportServer.ChangeRequestFieldUnderCurrentLanguage(TheTextOfTheRequest, "ConfigurationOptions.LongDesc");
				EndIf;
			EndIf;
