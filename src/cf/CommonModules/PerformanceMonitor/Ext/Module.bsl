...
//
Procedure EndTimeMeasurement(KeyOperation, BeginTime, MeasurementWeight = 1, Comment = Undefined, CompletedWithError = False) Export
	If PerformanceMonitorServerCallCached.RunPerformanceMeasurements() Then	
...
//             			- Map - 
//
Procedure EndTechnologicalTimeMeasurement(KeyOperation, BeginTime, MeasurementWeight = 1, Comment = Undefined) Export
	If PerformanceMonitorServerCallCached.RunPerformanceMeasurements() Then	
		EndTime = CurrentUniversalDateInMilliseconds();
		Duration = (EndTime - BeginTime)/1000;
...
Procedure CreateKeyOperations(KeyOperations) Export
	...
	Table.Columns.Add("KeyOperationName", New TypeDescription("String",,,,New StringQualifiers(1000)));
...
Procedure ChangeKeyOperations(KeyOperations) Export
	...
	Table = New ValueTable;
	Table.Columns.Add("NameOfKeyOperationIsOld", New TypeDescription("String",,,,New StringQualifiers(1000)));
	Table.Columns.Add("KeyOperationNameNew", New TypeDescription("String",,,,New StringQualifiers(1000)));
	Table.Columns.Add("ResponseTimeThreshold", New TypeDescription("Number",,,New NumberQualifiers(15,2)));
...
// Returns:
//   Map:
//   * KeyOperation - the name of the key operation.
//   * BeginTime - the start time of the key operation in milliseconds.
//   * LastMeasurementTime - the time of the last measurement of the key operation in milliseconds.
//   * MeasurementWeight - the amount of data processed during the execution of actions.
//   * NestedMeasurements - collection of measurements of nested steps.
//
	...
	For Each Measurement In MeasurementsArray Do
		...
		If Not ValueIsFilled(Measurement.KeyOperationStartDate) Then
			WriteLogEvent(NStr("en = 'Sampling error. Start date required';", 
				PerformanceMonitorInternal.DefaultLanguageCode()),
				EventLogLevel.Information,,,String(Measurement.KeyOperation));
			Return;
		EndIf;
		
		If TypeOf(Measurement.KeyOperation) = Type("String") Then
			KeyOperationRef = PerformanceMonitorCached.GetKeyOperationByName(Measurement.KeyOperation, Measurement.TimeConsuming);
		Else
...
Procedure PerformanceMonitorDataExport(DirectoriesForExport, AddlParameters = Undefined) Export
	...
	If AddlParameters = Undefined And Not PerformanceMonitorServerCallCached.RunPerformanceMeasurements() Then
	    Return;	
	EndIf;
...
Function ExportFileFullName(Directory, CurDate, FileSequenceNumber, ExtentionWithDot)
	...
	Return RemoveSeparatorsAtFileNameEnd(Directory, Separator) + Separator + Format(FileFormationDate, "DF='yyyy-MM-dd HH-mm-ex-" + FileSequenceNumberFormat + "'") + ExtentionWithDot;
...
Procedure LoadPerformanceMonitorFile(FileName, StorageAddress) Export
	...
	Try
		...
		For Each File In FindFiles(FilesDirectory, "*.XML") Do
			...
			LoadPerformanceMonitorFileApdexExport(XMLReader, AvailableKeyOperations, KeyOperationsToWrite, RawMeasurementsToWrite);  // @skip-
			XMLReader.Close();
		EndDo;
		DeleteFiles(FileForStorage);
		DeleteFiles(FilesDirectory);
