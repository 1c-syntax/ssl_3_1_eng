...
//   * UpdateListsOfFilesOnForms - Boolean - a flag showing whether updating file lists in open forms is required
//                                              to update data.
//
Function FileData(Val FileRef, Val VersionRef = Undefined, Val FileDataParameters = Undefined, Val GettingFileFromArchive = False) Export
	...
	FileData.Insert("FileName", CommonClientServer.GetNameWithExtension(
		FileObject1.Description, FileObject1.Extension));
	...
	If VersionRef = Undefined Then
		If GetBinaryDataRef Then
			FileBinaryData = FilesOperationsInternal.FileBinaryData(CurrentFileVersion, RaiseException1, GettingFileFromArchive);
			
			If GettingFileFromArchive Then
				FileBinaryData = WorkingWithServerFileArchive.BinaryDataOfFileExtractedFromZipArchive(FileBinaryData, FileRef);
			EndIf;
			
			RefToBinaryFileData = PutToTempStorage(FileBinaryData, FormIdentifier);
		Else
	...
	Else
		If GetBinaryDataRef Then
			FileBinaryData = FilesOperationsInternal.FileBinaryData(VersionRef, RaiseException1, GettingFileFromArchive);

			If GettingFileFromArchive Then
				FileBinaryData = WorkingWithServerFileArchive.BinaryDataOfFileExtractedFromZipArchive(FileBinaryData, VersionRef);
			EndIf;			
	...
	FileData.Insert("UpdateListsOfFilesOnForms", False);
...
// and as the FileData parameter value in other procedures and functions. See FilesOperations.FileData.
//
// Parameters: 
...
//  See FilesOperations.FileData.
//
Function GetFileData(Val AttachedFile, Val AdditionalParameters = Undefined) Export	

	Return FilesOperations.FileData(AttachedFile, AdditionalParameters);
EndFunction
...
Function FileDataToPrint(Val AttachedFile, Val FormIdentifier = Undefined) Export
	...
	FileData = GetFileData(AttachedFile, AdditionalParameters);
...
Function FileDataToOpen(FileRef, VersionRef, FormIdentifier = Undefined,
	OwnerWorkingDirectory = Undefined, FilePreviousURL = Undefined, FileGettingParameters = Undefined) Export
	...
	If TypeOf(FileGettingParameters) = Type("Structure") Then
		...
		FileDataParameters.FormIdentifier = FileGettingParameters.AddressInParentSession;
	...
	If CheckPresenceOfFileInArchive Then
		GettingFileFromArchive = FilesOperationsInternal.FileIsLocatedInFileArchive(FileRef, VersionRef);
	EndIf;
	
	FileData = FileData(FileRef, VersionRef, FileDataParameters, GettingFileFromArchive);
	...
	If FileData.Version <> Undefined And VersionInfo.FileStorageType
		...
		BinaryData = FilesOperationsInVolumesInternal.FileData(FileData.Version,,GettingFileFromArchive);
		If GettingFileFromArchive Then
			BinaryData = WorkingWithServerFileArchive.BinaryDataOfFileExtractedFromZipArchive(BinaryData, FileData.Version);
		EndIf;
		
		If TypeOf(FileGettingParameters) = Type("Structure") Then
			FormIdentifier = FileGettingParameters.AddressOfCurrentVersionInParentSession;
		EndIf;
...
Function StartActionWithFile(Val TimeConsumingOperationParameters, Val FunctionName) Export
	
	OwnerID = TimeConsumingOperationParameters.OwnerID;
	ExecutionParameters = TimeConsumingOperations.FunctionExecutionParameters(OwnerID);
	
	If FunctionName = "FilesOperationsInternal.FileDataToOpenAsynchronous" Then
		TimeConsumingOperation	= TimeConsumingOperations.ExecuteFunction(ExecutionParameters, 
			"FilesOperationsInternal.FileDataToOpenAsynchronous", TimeConsumingOperationParameters);
	ElsIf FunctionName = "FilesOperationsInternal.FileDataToSaveAsynchronous" Then
		TimeConsumingOperation	= TimeConsumingOperations.ExecuteFunction(ExecutionParameters, 
			"FilesOperationsInternal.FileDataToSaveAsynchronous", TimeConsumingOperationParameters);
	ElsIf FunctionName = "FilesOperationsInternal.FileDataEmailManagementAsynchronous" Then
		TimeConsumingOperation	= TimeConsumingOperations.ExecuteFunction(ExecutionParameters, 
			"FilesOperationsInternal.FileDataEmailManagementAsynchronous", TimeConsumingOperationParameters);
	Else
		Raise(StringFunctionsClientServer.SubstituteParametersToString(
				NStr("en = 'Unexpected file retrieval method: %1'"), FunctionName));
	EndIf;
...
// Retrieves a collection with the values of the attachment's Description and Extension attributes.
//
// Parameters:
//  AttachedFile	- DefinedType.AttachedFile - a reference to the catalog item with file.
//
// Returns:
//  Structure:
//   * Extension - String - Attachment's extension. See the AttachedFile.Extension attribute.
//   * FileName - String - Full name of the attachment. See the AttachedFile.Description and AttachedFile.Extension attributes.
//
Function FileNameAndExtension(Val AttachedFile) Export

	AttributesValues = Common.ObjectAttributesValues(AttachedFile, "Description,Extension");

	Result = New Structure;
	Result.Insert("Extension"	, AttributesValues.Extension);
	Result.Insert("FileName"	, CommonClientServer.GetNameWithExtension(AttributesValues.Description, AttributesValues.Extension));
...
Function RefreshFileObject(FileRef, FileInfo1, VersionRef = Undefined,
	...
	Try	
		...
		WorkingWithServerFileArchive.FillInFileStorageParameters(Version, BinaryData, FileInfo1.Size, Extension);		
		...
		Else // store in a volume
			...
			FilesOperationsInVolumesInternal.FillInTheFileDetails(Version, BinaryData,,,Version.Volume);
		EndIf;
		...
		If Version.Size = 0 And WorkingWithFilesInBinaryDataWarehouseIsService.StorageTypeDoesNotUseDisks(FileStorageType) Then
			FileBinaryData = FileStorage1.Get(); // BinaryData
			Version.Size = FileBinaryData.Size();
		EndIf;
...
Function CopyAttachedFile(SourceFile, NewFileOwner)
	...
	WorkingWithServerFileArchive.FillInFileStorageParameters(NewFile, BinaryData, NewFile.Size, NewFile.Extension);

	If WorkingWithFilesInBinaryDataWarehouseIsService.StorageTypeDoesNotUseDisks(NewFile.FileStorageType) Then
		RecordingParametersVIB = WorkingWithServerFileArchive.CompletedParametersOfEntryInInformationDatabase(NewFile);
		
		SetPrivilegedMode(True);
		InformationRegisters.FileRepository.WriteBinaryData(FileCopyRef, BinaryData, RecordingParametersVIB);
		SetPrivilegedMode(False);
	Else

		FilesOperationsInVolumesInternal.AppendFile(NewFile, BinaryData,,,NewFile.Volume);
...
//
// Parameters:
//  FileOrVersionRef - CatalogRef.Files - File reference.
//  AdditionalParameters	- See FilesOperationsClientServer.AdditionalParametersFileDataToSave.
//
// Returns:
//   Structure - structure with file data.
//
Function FileDataToSave(Val FileRef, Val AdditionalParameters = Undefined) Export

	If AdditionalParameters = Undefined Then
		AdditionalParameters = FilesOperationsClientServer.AdditionalParametersFileDataToSave();
	EndIf;
	
	FileData = FileDataToOpen(FileRef, 
											AdditionalParameters.FileVersion, 
											AdditionalParameters.FormIdentifier, 
											AdditionalParameters.OwnerWorkingDirectory,
											,
											AdditionalParameters.FileGettingParameters);
