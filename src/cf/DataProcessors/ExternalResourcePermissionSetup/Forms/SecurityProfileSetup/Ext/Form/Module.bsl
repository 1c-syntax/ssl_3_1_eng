		...
		If (PreviousMode <> NewMode) Then
			...
			If (PreviousMode = 2 Or NewMode = 2) Then
				...
				If NewMode = 2 Then
					...
					ExternalResourcePermissionSetupClient.StartEnablingSecurityProfilesUsage(ThisObject, ClosingNotification1);
					
				Else
					
					ExternalResourcePermissionSetupClient.StartDisablingSecurityProfilesUsage(ThisObject, ClosingNotification1);
...
Procedure RestoreSecurityProfiles(Command)
	...
	Try
		...
		StartApplyingSecurityProfilesSettings(ThisObject.UUID);
		ClosingNotification1 = New NotifyDescription("AfterCloseSecurityProfileCustomizationWizard", ThisObject, True);
		ExternalResourcePermissionSetupClient.StartRestoringSecurityProfiles(ThisObject, ClosingNotification1);
