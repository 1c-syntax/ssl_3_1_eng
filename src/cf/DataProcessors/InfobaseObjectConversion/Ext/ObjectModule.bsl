...
Function ExportByRule(
	...
	ElsIf OCR.Owner().Columns.Find("AnObjectWithMovements") <> Undefined
		And OCR.AnObjectWithMovements = True Then
	...
	If HasBeforeConvertObjectGlobalHandler Then
		...
		Except
			WriteInfoOnOCRHandlerExportError(64, ErrorProcessing.DetailErrorDescription(ErrorInfo()), OCR, Source, NStr("en = 'BeforeConvertObject (global)';"));
		EndTry;
	...
	If OCR.HasAfterExportToFileHandler Then
		...
		Except
			WriteInfoOnOCRHandlerExportError(79, ErrorProcessing.DetailErrorDescription(ErrorInfo()), OCR, Source, "HasAfterExportToFileHandler");
		EndTry;
...
Procedure ExternalConnectionConversionHandlerBeforeDataImport(Cancel) Export
	...
	If Not IsBlankString(Conversion.BeforeImportData) Then
		...
		Except
			WriteErrorInfoConversionHandlers(22, ErrorProcessing.DetailErrorDescription(ErrorInfo()), NStr("en = 'BeforeImportData (conversion)';"));
			Cancel = True;
		EndTry;
...
Procedure ExternalConnectionAfterDataImport() Export
	...
	If Not ErrorFlag() Then
		...
		If Not IsBlankString(Conversion.AfterImportData) Then
			...
			Except
				WriteErrorInfoConversionHandlers(23, ErrorProcessing.DetailErrorDescription(ErrorInfo()), NStr("en = 'AfterImportData (conversion)';"));
			EndTry;
...
Procedure InitializeKeepExchangeProtocol()
	...
	Except
		...
		MessageString = NStr("en = 'Failed to log to: %1. Error details: %2';",
			Common.DefaultLanguageCode());
		MessageString = StringFunctionsClientServer.SubstituteParametersToString(MessageString, ExchangeProtocolFileName, ErrorProcessing.DetailErrorDescription(ErrorInfo()));
...
Procedure ImportConversionRule(ExchangeRules, XMLWriter)
	...
	While ExchangeRules.Read() Do
		...
		ElsIf NodeName = "AnObjectWithMovements" Then
			
			NewRow.AnObjectWithMovements = deElementValue(ExchangeRules, BooleanType);
	...
	For Each TableRow In ConversionRulesCollection() Do
		...
		If ExchangeMode = "Upload0" And TableRow.Source <> Undefined Then
			...
			Try
				...
				EndIf;
			Except
				WriteErrorInfoToProtocol(11, ErrorProcessing.DetailErrorDescription(ErrorInfo()), String(TableRow.Source));
			EndTry;
...
Procedure StartReadMessage(MessageReader, DataAnalysis = False)
	...
	If Not IsBlankString(Conversion.BeforeImportData) Then
		...
		Except
			Raise WriteErrorInfoConversionHandlers(22, ErrorProcessing.DetailErrorDescription(ErrorInfo()), NStr("en = 'BeforeImportData (conversion)';"));
		EndTry;
	...
	Except
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("en = 'Cannot set a data exchange lock.
...
Procedure ExecuteHandlerAfterImportData()
	...
	If Not IsBlankString(Conversion.AfterImportData) Then
		...
		Except
			Raise WriteErrorInfoConversionHandlers(23, ErrorProcessing.DetailErrorDescription(ErrorInfo()), NStr("en = 'AfterImportData (conversion)';"));
		EndTry;
...
Function ReadRegisterRecordSet()
	...
	If HasBeforeImportObjectGlobalHandler Then
		...
		Except
			...
			WriteInfoOnOCRHandlerImportError(53, ErrorProcessing.DetailErrorDescription(ErrorInfo()), RuleName, Source, 
		...
		ElsIf NodeName = "TabularSection"
			...
			ElsIf NodeName = "RecordSet" Then
				...
				If Metadata.AccountingRegisters.Find(Name) <> Undefined Then
					...
					LoadedMovements = Object.RegisterRecords[Name].Unload();
					If LoadedMovements.Count() > 0 Then
						
						// 
						// 
						// 
						EliminateErrorOfAssigningTypeOfSubconto(LoadedMovements);
						
						DeferredMotionRecordingTables.Insert(Name, LoadedMovements);
		...
		ElsIf (NodeName = "Object") And (ExchangeFile.NodeType = XMLNodeTypeEndElement) Then
			...
			If HasAfterObjectImportGlobalHandler Then
				...
				Except
					...
					WriteInfoOnOCRHandlerImportError(54, ErrorProcessing.DetailErrorDescription(ErrorInfo()), RuleName, Source,
...
Procedure CheckOutSubKontoCollection(TableRow, ValuesBeforeCorrection, Direction, HasError)
	
	CollectionOfSubKontoSpecies = TableRow["Account" + Direction].ExtDimensionTypes;
	
	For Each ViewOfSubKontoFromCollection In CollectionOfSubKontoSpecies Do
		
		ExtDimensionType = Direction + String(ViewOfSubKontoFromCollection.LineNumber);
		...
		If TableRow[NameTypesOfSubconto] <> ViewOfSubKontoFromCollection.ExtDimensionType Then
			
			DescriptionOfExtdimension.HasError = True;
			
			TableRow[NameExtdimension] = Undefined;
			TableRow[NameTypesOfSubconto] = ViewOfSubKontoFromCollection.ExtDimensionType;
...
Procedure EliminateErrorOfAssigningTypeOfSubconto(LoadedMovements)
	
	For Each TableRow In LoadedMovements Do
		
		HasError = False;
		
		ValuesBeforeCorrection = New Map;
		
		CheckOutSubKontoCollection(TableRow, ValuesBeforeCorrection, "Dr", HasError);
		CheckOutSubKontoCollection(TableRow, ValuesBeforeCorrection, "Cr", HasError);
...
Procedure FireEventsBeforeExportObject(Object, Rule, Properties=Undefined, IncomingData=Undefined, 
	...
	If HasBeforeExportObjectGlobalHandler Then
		...
		Except
			WriteErrorInfoDERHandlers(65, ErrorProcessing.DetailErrorDescription(ErrorInfo()), Rule.Name, NStr("en = 'BeforeExportSelectionObject (global)';"), Object);
		EndTry;
...
Procedure FireEventsAfterExportObject(Object, Rule, Properties=Undefined, IncomingData=Undefined, 
	...
	If HasAfterExportObjectGlobalHandler Then
		...
		Except
			WriteErrorInfoDERHandlers(69, ErrorProcessing.DetailErrorDescription(ErrorInfo()), Rule.Name, NStr("en = 'AfterExportSelectionObject (global)';"), Object);
		EndTry;
	EndIf;
...
Procedure ProcessObjectDeletion(ObjectDeletionData, ErrorMessageString = "")
	...
	If Conversion.Property("BeforeSendDeletionInfo", EventText) Then
		...
		If Not IsBlankString(EventText) Then
			...
			Except
				ErrorMessageString = WriteErrorInfoConversionHandlers(76, ErrorProcessing.DetailErrorDescription(ErrorInfo()), NStr("en = 'BeforeSendDeletionInfo (conversion)';"));
	...
	Rules = ConversionRulesCollection().Copy(Filter, "DestinationType,SourceType");
	Rules.GroupBy("DestinationType, SourceType");
...
Procedure InitConversionRuleTable()
	...
	Columns.Add("AnObjectWithMovements", deTypeDetails("Boolean"));
...
Procedure InitMessages()
	...
	ErrorsMessages.Insert(6,  NStr("en = 'An error occurred when importing exchange rules';"));
	ErrorsMessages.Insert(7,  NStr("en = 'Exchange rule format error';"));
	ErrorsMessages.Insert(8,  NStr("en = 'File name for data export is specified incorrectly';")); // 
	...
	ErrorsMessages.Insert(11, NStr("en = 'Exchange rules contain a reference to a non-existing metadata object';"));
	ErrorsMessages.Insert(12, NStr("en = 'Exchange rules file name is not specified.';"));
			
	ErrorsMessages.Insert(13, NStr("en = 'An error occurred when retrieving an object property value by a source property name.';"));
	ErrorsMessages.Insert(14, NStr("en = 'An occurred when retrieving an object property value by a destination property name.';"));
	
	ErrorsMessages.Insert(15, NStr("en = 'Data import file name is not specified.';"));
			
	ErrorsMessages.Insert(16, NStr("en = 'An error occurred when retrieving a subordinate object property value by a source property name.';"));
	ErrorsMessages.Insert(17, NStr("en = 'Cannot get a value of a subordinate object property by destination property name.';"));
	ErrorsMessages.Insert(18, NStr("en = 'Cannot create a data processor with handlers code.';"));
	...
	ErrorsMessages.Insert(45, NStr("en = 'No object conversion rule is found';"));
	...
	ErrorsMessages.Insert(66, NStr("en = 'Cannot get a collection of subordinate objects from incoming data.';"));
	ErrorsMessages.Insert(67, NStr("en = 'Cannot get a property of a subordinate object from incoming data.';"));
	ErrorsMessages.Insert(68, NStr("en = 'Cannot get an object property from incoming data.';"));
	
	ErrorsMessages.Insert(69, NStr("en = 'Global event handler error (data conversion): AfterExportObject';"));
	
	ErrorsMessages.Insert(71, NStr("en = 'Cannot find a mapping for the source value.';"));
	
	ErrorsMessages.Insert(72, NStr("en = 'Cannot export data for the exchange plan node.';"));
...
Procedure ImportExchangeRules(Source="",
	...
	If ExchangeMode <> "Load" And Conversion.Property("AfterImportExchangeRules", AfterExchangeRulesImportEventText)
		...
		Except
			ErrorMessageString = WriteErrorInfoConversionHandlers(75, ErrorProcessing.DetailErrorDescription(ErrorInfo()), NStr("en = 'AfterImportExchangeRules (conversion)';"));
...
Procedure DeleteObjectByLink(Ref, ErrorMessageString)
	...
	If Not IsBlankString(Conversion.OnGetDeletionInfo) Then
		...
		Except
			ErrorMessageString = WriteErrorInfoConversionHandlers(77, ErrorProcessing.DetailErrorDescription(ErrorInfo()), NStr("en = 'OnGetDeletionInfo (conversion)';"));
			Cancel = True;
				...
				Try
					...
					If Cancel = True Then
						...
						If Not IsBlankString(CancelReason) Then
							...
							MessageString = NStr("en = 'The data import is canceled. Reason: %1';");
							MessageString = StringFunctionsClientServer.SubstituteParametersToString(MessageString, CancelReason);
							Raise MessageString;
						Else
							Raise NStr("en = 'The data import is canceled.';");
						EndIf;
...
Procedure ReadDataViaExchange(MessageReader, DataAnalysis)
	...
	If DataImportToInfobaseMode() Then
		...
		If DeleteChangeRecords Then // 
			...
			If TransactionActive() Then
				Raise NStr("en = 'Cannot delete the registration of data changes in an active transaction.';");
			EndIf;
	...
	If Not IsBlankString(Conversion.AfterGetExchangeNodesInformation) Then
		...
		Except
			Raise WriteErrorInfoConversionHandlers(176, ErrorProcessing.DetailErrorDescription(ErrorInfo()), NStr("en = 'AfterGetExchangeNodesInformation (conversion)';"));
		EndTry;
...
Procedure ExecuteExport(ErrorMessageString = "")
	...
	If Not IsBlankString(Conversion.BeforeExportData) Then
		...
		Except
			WriteErrorInfoConversionHandlers(62, ErrorProcessing.DetailErrorDescription(ErrorInfo()), NStr("en = 'BeforeExportData (conversion)';"));
			Cancel = True;
		EndTry; 
	...
	If Not Cancel And Not IsBlankString(Conversion.AfterExportData) Then
		...
		Except
			WriteErrorInfoConversionHandlers(63, ErrorProcessing.DetailErrorDescription(ErrorInfo()), NStr("en = 'AfterExportData (conversion)';"));
		EndTry;
...
Procedure ExecuteRegisteredDataExport(WriteMessage1, ErrorMessageString, UsedExportRulesTable)
	...
	If Not IsBlankString(Conversion.BeforeGetChangedObjects) Then
		...
		Except
			WriteErrorInfoConversionHandlers(175, ErrorProcessing.DetailErrorDescription(ErrorInfo()), NStr("en = 'BeforeGetChangedObjects (conversion)';"));
			Return;
		EndTry;
	...
	Try
			...
			If MetadataObjectPrevious <> CurrentMetadataObject Then
				...
				If MetadataObjectPrevious <> Undefined Then
					...
					If PreviousDataExportRule <> Undefined
						...
						Except
							WriteErrorInfoDERHandlers(32, ErrorProcessing.DetailErrorDescription(ErrorInfo()), PreviousDataExportRule["Name"], "AfterProcessDataExport");
						EndTry;
				...
				If DataExportRule <> Undefined
					...
					Except
						WriteErrorInfoDERHandlers(31, ErrorProcessing.DetailErrorDescription(ErrorInfo()), DataExportRule.Name, "BeforeProcessDataExport");
					EndTry;
		...
		If MetadataObjectPrevious <> Undefined Then
			...
			If DataExportRule <> Undefined
				...
				Except
					WriteErrorInfoDERHandlers(32, ErrorProcessing.DetailErrorDescription(ErrorInfo()), DataExportRule.Name, "AfterProcessDataExport");
				EndTry;
