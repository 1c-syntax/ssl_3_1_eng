...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	DetailsMode = (Parameters.Property("Details") And Parameters.Details <> Undefined)
		Or Parameters.Property("VariantPresentation") And Parameters.VariantPresentation = "Details";
...
Procedure ChoiceProcessing(Result, SubordinateForm)
	...
	If TypeOf(SubordinateForm) = Type("ClientApplicationForm") Then
		...
		ElsIf TypeOf(Result) = Type("Structure") Then
			...
			If FormSourceName = Upper("ReportSettingsForm")
				Or FormSourceName = Upper("SettingsForm")
				Or FormSourceName = Upper("ReportVariantForm")
				Or FormSourceName = Upper("VariantForm") Then
	...
	If CommonClient.SubsystemExists("StandardSubsystems.ReportMailing") Then
		...
		ModuleReportDistributionClient.ChoiceProcessingReportForm(ThisObject, Result, SubordinateForm, ResultProcessed);
	EndIf;
...
Procedure NotificationProcessing(EventName, Parameter, Source)
	...
	ElsIf EventName = "ChangeReportOptionVisibilityInReportPanel"
	        And Parameter = ReportSettings.FullName
	...
	SSLSubsystemsIntegrationClient.OnProcessNotification(ThisObject, EventName, Parameter, Source, NotificationProcessed);
...
Procedure BeforeLoadVariantAtServer(NewDCSettings)
	...
	If NewDCSettings = Undefined
		Or Not ReportsOptionsInternalClientServer.ReportOptionMode(CurrentVariantKey) Then
...
Procedure OnLoadVariantAtServer(NewDCSettings)
	// If the report is not in DCS and settings are not imported, do nothing.
	If Not ReportsOptionsInternalClientServer.ReportOptionMode(CurrentVariantKey)
		And NewDCSettings = Undefined Then
	...
	If DetailsMode Then
		...
		If Parameters <> Undefined
			And Parameters.Property("Details")
			And TypeOf(Parameters.Details) = Type("DataCompositionDetailsProcessDescription") Then
...
&AtClient
Procedure FactorExtendedTooltipURLProcessing(Item, FormattedStringURL, StandardProcessing)
...
Procedure ReportSpreadsheetDocumentSelection(Item, Area, StandardProcessing)
	...
	If StandardProcessing Then
		SSLSubsystemsIntegrationClient.OnProcessSpreadsheetDocumentSelection(ThisObject, Item, Area, StandardProcessing);
		ReportsClientOverridable.SpreadsheetDocumentSelectionHandler(ThisObject, Item, Area, StandardProcessing);
	EndIf;
	
	If Not DetailProcessing
		And StandardProcessing
		And TypeOf(Area) = Type("SpreadsheetDocumentRange") Then
		...
		If DetailsValue <> Undefined
			And GoToLink(DetailsValue) Then
	...
	If Not DetailProcessing
		...
		ReportsOptionsInternalClient.ShowTheContextSettingOfTheReport(ThisObject, Item, Area, StandardProcessing);
	EndIf;
...
Procedure ReportSpreadsheetDocumentAdditionalDetailProcessing(Item, Details, StandardProcessing)
	...
	SSLSubsystemsIntegrationClient.OnProcessAdditionalDetails(ThisObject, Item, Details, StandardProcessing);
	...
	If StandardProcessing Then 
		...
		ReportsOptionsInternalClient.AdditionalDetailProcessing(ThisObject, Data, Item, Details, StandardProcessing);
	EndIf;
...
Procedure DefaultSettings(Command)
	...
	ParametersForm.Insert("ResetSettings", True);
...
Procedure ChangeQuickSettingsComposition(Command)
	...
	OpenForm("SettingsStorage.ReportsVariantsStorage.Form.QuickReportSettings",
...
Procedure SaveReportOptionToFile(Command)
	OpenForm(
		...
		ReportsOptionsInternalClient.ParametersForSavingAReportVariantToAFile(ThisObject),
		ThisObject);
EndProcedure
...
Procedure Attachable_ImportReportOption(Command)
	...
	ParametersForm.Insert("ResetSettings", True);
	LoadVariant(FormOption.VariantKey, False);  
...
Procedure DefineTheBehaviorOnTheHomePage()
	...
	For Each TheFormOfTheInitialPage In HomePageWindow.Content Do 
		...
		If TheFormOfTheInitialPage.FormName = FormName
			And TheFormOfTheInitialPage.UniqueKey = UniqueKey Then 
	...
	CommonClientServer.SetFormItemProperty(Items, "GroupSaveReportOptionSelection", "Visible", False);
...
Procedure Generate()
	...
	If Result.Property("StorageParameterNameWarningDisclaimer") Then
		...
		If ValueIsFilled(Result.StorageParameterNameWarningDisclaimer) Then
			LayoutParameter = New DataCompositionParameter(
				Result.StorageParameterNameWarningDisclaimer);
		...
		If Warn Then
			...
			Buttons.Add("Continue", NStr("en = 'Resume';"));
...
Procedure GenerateAfterWarning(Response, Context) Export
	...
	ElsIf TypeOf(Response) = Type("Structure") Then
		If Response.NeverAskAgain
		   And Context.WarningDisclaimerSetting <> Undefined Then
		...
		If Response.Value = "Continue" Then
			...
			Result = ReportGenerationResult(GeneratingOnOpen,
				ReportSettings.External Or ReportSettings.Safe, True);
		Else
			Return;
		EndIf;
	Else
...
Procedure AfterGenerate(Result, GenerationParameters) Export 
	...
	If Result = Undefined Then 
		...
		ShowUserNotification(NStr("en = 'Report is not generated';"), Window.GetURL(), Title);
	...
	ElsIf Result.Status = "Completed2" Then 
		...
		ShowUserNotification(NStr("en = 'Report is generated';"), Window.GetURL(), Title);
...
Function ReportGenerationParameters(ReportName, Directly)
	...
	ReportGenerationParameters.Insert("OptionRef1", ReportSettings.OptionRef);
	...
	ReportGenerationParameters.Insert("TablesToUse", ReportSettings.TablesToUse);
	ReportGenerationParameters.Insert("ParametersChanged", ParametersChanged);
...
Procedure ShowChoiceList(Item, StandardProcessing)
	...
	ExtendedTypesDetails = CommonClientServer.StructureProperty(
		Report.SettingsComposer.Settings.AdditionalProperties,
		"ExtendedTypesDetails");
	...
	ValuesForSelection = ValuesForSelection(
		Item.ChoiceList,
		InformationRecords.UserSettingItem,
		Item.AvailableTypes,
		AvailableValues);
	...
	If ReportsClient.ChoiceOverride(ThisObject, Handler, InformationRecords.LongDesc,
			Item.AvailableTypes, MarkedValues, ChoiceParameters) Then
		Return;
	EndIf;
	
	If ReportsClient.IsSelectMetadataObjects(Item.AvailableTypes, MarkedValues, Handler)
		Or ReportsClient.IsSelectUsers(ThisObject, Item, Item.AvailableTypes, MarkedValues, ChoiceParameters, Handler) Then 
		Return;
	EndIf;
...
Procedure FindSuitableValuesOfTheUniversalNumberSearch(SuitableValues, SearchFieldsByType, SearchParameters)
	...
	AvailableSearchFields = AvailableNumberSearchFields(SearchFields, SearchParameters.SearchNumber, SearchParameters.ControlCharacters);
	
	For Each FieldForSearch In AvailableSearchFields Do 
		
		SearchProperties = UniversalSearchProperties(FieldForSearch.Key, SearchParameters.SearchNumber, SearchParameters.ControlCharacters);
	...
	ReportsClientServer.AddToList2(SuitableValues, SuitableValuesForTheNumber);
...
Procedure FindSuitableUniversalSearchValues(SuitableValues, SearchFieldsByType, SearchParameters)
	...
	While Selection.Next() Do 
		...
		For Each FieldForSearch In SearchFields Do 
			...
			SearchProperties = UniversalSearchProperties(FieldForSearch.Key, Selection.Ref, SearchParameters.ControlCharacters);			
			...
			SearchOptions = New Array;
			SearchOptions.Add(SearchProperties);			
			AdditionalValues.Add(SearchOptions, 
				ViewSearchResult(FieldForSearch.Value, SearchProperties.ComparisonType, 
	...
	ReportsClientServer.AddToList2(SuitableValues, AdditionalValues);
...
Function TheTextOfTheRequestForTheSearchObject(Object, SearchParameters)
	...
	QueryText = StrReplace(QueryText, "&FieldByCondition", TheFieldOfTheQueryTextForTheSearchObject(Object, SearchParameters));
...
Function TheFieldOfTheQueryTextForTheSearchObject(Object, SearchParameters)
	...
	For Each Field In Fields Do
		...
		AttributeType = SearchAttributeType(Object, Field);
		...
		FieldByCondition = StringFunctionsClientServer.SubstituteParametersToString(
			"WHEN CAST(%1 AS STRING(150)) LIKE %2 ESCAPE ""~"" THEN ""%3""",
			Field.Name,
	...
	For Each Field In Fields Do
		...
		If AttributeType.ContainsType(Type("String")) Then 
			...
			Condition = StringFunctionsClientServer.SubstituteParametersToString(
				"CAST(%1 AS STRING(150)) LIKE %2 ESCAPE ""~""",
				Field.Name,
				"""%" + Common.GenerateSearchQueryString(SearchParameters.TheSearchStringIsNormalized) + "%""");
				
		ElsIf AttributeType.ContainsType(Type("Number"))
			And SearchParameters.SearchNumber <> Undefined Then  
			
			Condition = StringFunctionsClientServer.SubstituteParametersToString(
				"CAST(%1 AS NUMBER) = &SearchNumber", Field.Name); //@query-part
...
//
&AtServerNoContext
Function SearchAttributeType(Object, Field)
	
	For Each AttributeDetails In Object.StandardAttributes Do 
		
		If AttributeDetails.Name = Field.Name Then 
			Return AttributeDetails.Type;
		EndIf;
		
	EndDo;
	
	AttributeDetails = Object.Attributes.Find(Field.Name);
...
Function RepresentationOfTheUniversalLinkSearch(Data, FieldForSearch, Condition, SearchString)
	...
	Return New FormattedString(FieldForSearch, ConditionFragment, FragmentBeforeTheSearchBar,
		FragmentOfTheSearchString, TheFragmentAfterTheSearchString);
...
Function ConditionFragmentTemplate(Condition)
	...
	Return StringFunctionsClientServer.SubstituteParametersToString(
		NStr("en = '%1<span style=""color: %2"">%3</span>%4';"),
		" ",
		"HiddenReportOptionColor",
...
Function SearchStringFragmentTemplate(SearchString)
	...
	Return StringFunctionsClientServer.SubstituteParametersToString(
		NStr("en = '<span style=""color: %1; font: %2"">%3</span>';"),
...
Function UniversalSearchCondition(SearchValue, ControlCharacters)
	...
	If TypeOf(SearchValue) = Type("String")
		And (Not ValueIsFilled(ControlCharacters)
		Or ControlCharacters = "=") Then 
		
		Condition = DataCompositionComparisonType.Contains;
		
	ElsIf TypeOf(SearchValue) = Type("String")
		And (ControlCharacters = "-"
		Or ControlCharacters = "<>") Then 
		
		Condition = DataCompositionComparisonType.NotContains;
		
	ElsIf ControlCharacters = "-"
		Or ControlCharacters = "<>" Then 
...
Function UniversalSearchFieldsByType(SchemaURL, Val Settings)
	...
	Collections.Add(DescriptionOfReportParameters(SettingsComposer));
...
&AtServerNoContext
Function DescriptionOfReportParameters(SettingsComposer)
	
	ParametersDetails = New Array;
	
	Settings = SettingsComposer.Settings;
	DataParameterFields = Settings.DataParametersAvailableFields.Items;
	DataParameters = Settings.DataParameters;
	
	For Each DataParametersField In DataParameterFields Do 
		
		For Each Item In DataParametersField.Items Do 
			
			FieldStructure = StrSplit(Item.Field, ".");
			
			If FieldStructure.Count() < 2 Then 
				Continue;
			EndIf;
			
			ParameterName = FieldStructure[1];
			
			If DataParameters.Items.Find(ParameterName) = Undefined Then 
				Continue;
			EndIf;
			
			ParameterDetails = DataParameters.AvailableParameters.Items.Find(ParameterName);
			
			If ParameterDetails = Undefined
				Or Not ParameterDetails.Visible Then 
				
				Continue;
			EndIf;
			
			ParametersDetails.Add(Item);
...
Procedure RefineFilterProperties(Filter, Search)
	...
	If Filter.Use
		...
		And Search.ComparisonType = DataCompositionComparisonType.Equal
		And Values.Count() > 1 Then 
		
		Filter.ComparisonType = DataCompositionComparisonType.InList;
		Filter.RightValue = Values;
		
	ElsIf Filter.Use
		And Filter.ComparisonType = DataCompositionComparisonType.InList
		And Search.ComparisonType = DataCompositionComparisonType.Equal Then 
		
		Filter.RightValue = Values;
		
	ElsIf Filter.Use
		And Filter.ComparisonType = DataCompositionComparisonType.NotEqual
		And Search.ComparisonType = DataCompositionComparisonType.NotEqual
		And Values.Count() > 1 Then 
	...
	ElsIf Filter.Use
		And Filter.ComparisonType = DataCompositionComparisonType.NotInList
...
Procedure FillInTheMenuOfGroupingLevels(IndexOfTheReportStructure)
	...
	If IndexOfTheReportStructure = Undefined
		Or IndexOfTheReportStructure.Count() = 0 Then 
	...
	For Each Properties In MenuProperties Do 
		...
		// 
		Button = Items.Add(Properties.CommandName + "More", Type("FormButton"), 
			Items.GroupsLevelsGroupMore);
		Button.Type = FormButtonType.CommandBarButton;
		Button.CommandName = Properties.CommandName;
		Button.Title = Properties.RepresentationOfTheGroupingLevel;
...
&AtServer
Function ReportGroupingsBasedOnAFieldWithAHierarchicalGrouping(IndexOfTheReportStructure, FieldsWithHierarchicalGrouping, NumberOfGroupingLevels)
	...
	If FieldsWithHierarchicalGrouping.Count() <> 1
		Or TheNumberOfLevelsOfTheFieldWithHierarchicalGrouping = 0 Then 
	...
	While TheNumberOfLevelsOfTheFieldWithHierarchicalGrouping > 0 Do 
		...
		AdditionalGrouping.PerformanceGroup = StringFunctionsClientServer.SubstituteParametersToString(
			GroupingViewTemplate,
			AdditionalGrouping.PerformanceGroup,
	...
	FoundGrouping.PerformanceGroup = StringFunctionsClientServer.SubstituteParametersToString(
		GroupingViewTemplate,
		FoundGrouping.PerformanceGroup,
		FieldLevelWithHierarchicalGrouping);
...
Procedure ShowTheSelectedGroupingLevel(GroupingLevel = Undefined)
	...
	MenuOutputAreas.Insert(Items.GroupsLevelsGroupContextMenu, Items.ShowGroupsLevelContextMenu);
...
&AtServerNoContext
Procedure SaveTheStateOfTheOptionOutputSettingsHeaders(ValueStorageKey, OutputSettingsTitles, DetailsMode)
...
&AtServerNoContext
Procedure RestoreTheStateOfTheOutputSettingsHeadersOption(ValueStorageKey, OutputSettingsTitles, DetailsMode)
...
Procedure SaveReportAfterFilenameSelected(Result, Context) Export 
	...
	If Not ValueIsFilled(FullNameOfTheReportFile) Then
		Context.Insert("FullNameOfTheReportFile", CommonClientServer.ReplaceProhibitedCharsInFileName(Title));
		...
		FormatsList.ShowChooseItem(OnCloseNotifyDescription, NStr("en = 'Select a save format';"), DefaultFormat);
	Else
...
Procedure AfterSaveFormatSelected(SelectedElement, Context) Export
	...
	Handler = New NotifyDescription("SaveReportAfterSavedReportResults", ThisObject, FullNameOfTheReportFile);
	
	SaveFormats = ReportSavingFormats(FullNameOfTheReportFile,
	Context.IndexOfReportSavingFormats);
	ResultProperties = ReportSettings.ResultProperties; // See ReportsOptionsInternal.PropertiesOfTheReportResult
...
Function AvailableFormatsForSavingTheReport(IndexOfReportSavingFormats)
	...
	ANSITXTSaveFormat = SaveFormats.Find(SpreadsheetDocumentFileType.ANSITXT, "SpreadsheetDocumentFileType");
	
	If SavingFormatTXT <> Undefined
		And ANSITXTSaveFormat <> Undefined Then 
	...
	For Each SaveFormat In SaveFormats Do
		...
		PresentationOfTheSaveFormat = StrReplace(SaveFormat.Presentation,
			"." + SaveFormat.Extension, ExtensionOfTheSaveFormat);
...
Procedure SetVisibilityAvailability()
	...
	ShowOptionsSelectionCommands =
		ReportsOptionsInternalClientServer.ReportOptionMode(CurrentVariantKey)
	...
	Items.ChangeQuickSettingsComposition.Visible = ShowOptionChangingCommands
	...
	IsPredefinedOption = Not ReportSettings.Custom
	...
	If ItIsAvailableToSaveTheCurrentVersion
	...
	Items.OtherReports.Visible = ReportSettings.OptionSelectionAllowed
		And Not ValueIsFilled(OptionContext);
	...
	CommonClientServer.SetFormItemProperty(
		Items, "ShareSettings", "Visible", UseSettingsAllowed
			And (IsPredefinedOption
				Or ReportsOptions.FullRightsToOptions()
				Or ValueIsFilled(ReportSettings.OptionRef)
				  And Common.ObjectAttributeValue(ReportSettings.OptionRef,
						"AuthorOnly") <> True));
...
Procedure LoadVariant(VariantKey, ClearStackSettings = True)
	If Not DetailsMode And Not VariantModified Then
		ObjectKey = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("en = '%1/%2/%3';"),
			ReportSettings.FullName,
			CurrentVariantKey,
			"CurrentUserSettings");
		
		Common.SystemSettingsStorageSave(
			ObjectKey, "", Report.SettingsComposer.UserSettings);
	EndIf;
...
Function ReportOptionContext()
	...
	If Parameters.Property("OptionContext")
		And Parameters.OptionContext <> Undefined Then 
	...
	For Each InterfaceProperty In InterfaceProperties Do 
		...
		ElsIf TypeOf(PropertyValue) = Type("Structure") Then 
			...
			For Each StructureItem In PropertyValue Do 
				...
				ElsIf TypeOf(ElementValue) = Type("Array")
					And ElementValue.Count() > 0
	...
	If TypeOf(CommandDetails) = Type("Structure") Then 
		...
		ContextTypes = ?(TypeOf(ContextTypeDetails) = Type("TypeDescription"), ContextTypeDetails.Types(), New Array);
...
Procedure SetPurposeUseKey()
	...
	ElsIf ValueIsFilled(OptionContext) Then 
		PurposeUseKey = OptionContext
			+ ?(ValueIsFilled(Parameters.VariantKey), ".", "")
...
Procedure SetCurrentOptionKey(ReportFullName, ReportObject)
	...
	If TypeOf(Details) = Type("DataCompositionDetailsProcessDescription") Then
		...
		CurrentVariantKeyDetail = CommonClientServer.StructureProperty(Settings.AdditionalProperties, "VariantKey", "");
	EndIf;
	...
	If Not ValueIsFilled(CurrentVariantKey)
		Or (ValueIsFilled(Parameters.VariantKey) And CurrentVariantKey <> Parameters.VariantKey) Then 
	...
	If ValueIsFilled(OptionContext)
		And ValueIsFilled(Parameters.VariantKey)
...
Function EventsSettings()
	...
	FindEventSettings(
		Items.ReportSpreadsheetDocument.ContextMenu,
		RefinementsEventSettings(),
		EventSettingsExceptions(),
		EventsSettings);
	
	EventsSettings.Insert("FilterAndGenerate", NStr("en = 'Filter…';"));
	
	EventsSettings.Insert(
...
Procedure FindEventSettings(Menu, Clarifications, Exceptions, EventsSettings)
	...
	For Each Item In Menu.ChildItems Do 
		...
		ElsIf EventsSettings[Item.CommandName] = Undefined
			...
			CommandAction = ?(ValueIsFilled(FoundCommand.Action), FoundCommand.Action, FoundCommand.Name);
...
Procedure SetUserPermissions()
	...
	If ReportSettings.EditOptionsAllowed
		And Not ReportsOptionsCached.InsertRight1() Then
...
Procedure ImportSettingsToComposer(ImportParameters)
	...
	ReportsServer.FillinAdditionalProperties(ReportObject,
		...
		?(ParametersForm.InitialOptionKey = CurrentVariantKey, OptionContext, ""),
		?(ParametersForm.InitialOptionKey = CurrentVariantKey, ParametersForm.Filter, Undefined));
				
	If AvailableSettings.Settings <> Undefined And ReportSettings.Events.BeforeImportSettingsToComposer Then      
		If CommonClientServer.StructureProperty(ParametersForm, "ResetSettings", False) Then
			AvailableSettings.Settings.AdditionalProperties.Insert("ResetSettings", True);	
		EndIf;
		...
		ParametersForm.Insert("ResetSettings", False);  
		AvailableSettings.Settings.AdditionalProperties.Insert("ResetSettings", False);
	EndIf;
	...
	If Not ReportSettings.Custom Then
		ReportObject.SettingsComposer.Settings.AdditionalProperties.Property(
			"DescriptionOption", ReportCurrentOptionDescription);
	EndIf;
	
	ReportsServer.SetAvailableValues(ReportObject, ThisObject);
	ReportsServer.InitializePredefinedOutputParameters(
		ReportSettings,
		Report.SettingsComposer.Settings,
...
Procedure ShowGenerationErrors(ErrorInfo)
	If TypeOf(ErrorInfo) = Type("ErrorInfo") Then
		...
		DetailErrorDescription = NStr("en = 'The report is not generated due to:';") 
	...
	If Not IsBlankString(DetailErrorDescription) Then
		ReportsOptions.WriteToLog(EventLogLevel.Warning, DetailErrorDescription, ReportSettings.OptionRef);
	EndIf;
EndProcedure
...
Function IsOptionMustBeSaved()
	...
	If AdditionalProperties.Property("FormParametersSelection")
	   And ValueIsFilled(AdditionalProperties.FormParametersSelection)
	 Or DetailsMode Then
...
Procedure SaveAsNewOrOverwriteExistingReportOptionAndContinue(Command)
	...
	If Not Items.SaveOptionAs.Visible Then
		ShowMessageBox(,
			NStr("en = 'Cannot perform the action as saving report options is unavailable.';"));
		Return;
	EndIf;
	...
	If Items.SaveVariant.Visible Then
		...
		StandardSubsystemsClient.ShowQuestionToUser(
			...
			NStr("en = 'Do you want to save the report option?';"),
			Buttons,
			QuestionParameters);
		Return;
	Else
		...
		OpenForm("SettingsStorage.ReportsVariantsStorage.SaveForm",
			FormParameters, ThisObject,,,, New NotifyDescription(
				"Attachable_CommandAfterOptionSaved", ThisObject, Command));
		Return;
	EndIf;
...
Procedure Attachable_CommandAfter(Response, Command) Export
	...
	If Response = Undefined
	 Or Response.Value <> "SaveAndResume" Then
		Return;
	EndIf;
...
Procedure SaveReportOption(AfterAddedNew = False)
	...
	ShowUserNotification(NStr("en = 'The report option is saved';"), Window.GetURL(), Title);
...
Procedure SaveReportOptionAtServer(AfterAddedNew = False)
	...
	SettingsStorages.ReportsVariantsStorage.Save(
		ReportKey,
		CurrentVariantKey,
		DCSettings,
		SettingsDescription);
...
Procedure UpdateOptionsSelectionCommands()
	...
	For Each TableRow In VariantsTable Do
		...
		Else
			...
			// 
			MoreButton = Items.Add(FormOption.CommandName + "More", Type("FormButton"), Items.MoreCommandBarReportOptionsGroup);
...
#If ThickClientOrdinaryApplication Or ThickClientManagedApplication Then
	...
	FillParameters.Insert("UserSettingsModified", True);
	FillParameters.Insert("DCSettingsComposer", Report.SettingsComposer);
	FillParameters.Insert("EventName", "DefaultSettings");
...
Procedure AfterChangingTheCompositionOfTheQuickSettings(Result, AdditionalParameters) Export 
	...
	If Result.VariantModified
		Or Result.OutputSettingsTitles <> OutputSettingsTitles Then 
...
Procedure UpdateKeyboardCommandTransitionBetweenSettingChanges()
	...
	If CurrentStackSettings.Count() > 0 Then 
		...
		If Not KeyboardShortcutSet
			And OrderCommandCurrentStackSettings = 1
	...
	ElsIf StackSettings.Count() > 0
		And ButtonSetStandardSettings.Check Then 
...
Function ListOfAvailableReportSaveFormats(IndexOfReportSavingFormats)
	...
	For Each SaveFormat In IndexOfReportSavingFormats Do
		...
		AvailableFormats.Add(FoundDetails.Extension, FoundDetails.Presentation, , FoundDetails.Picture);
	EndDo;
